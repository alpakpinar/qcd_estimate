...
def main():
    ...
    for binning in ['nom']:
        for region in regions:
            try:
                _inpath = pjoin(inpath, region)
                plot_qcd(_inpath, fit='nominal', binning=binning, region=region)
                plot_qcd(_inpath, fit='nominal', binning=binning, cr_only=True, region=region)
    
                plot_data_driven_qcd_in_cr(_inpath, fit='nominal', binning=binning, region=region)
            except KeyError:
